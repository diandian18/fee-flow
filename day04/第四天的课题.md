...
- 对我们的架构来说，kafka可以把产生日志的`nginx`和消费服务`task server`解耦开来，分别部署。
...
3. 消息：kafka会保存消息直到它过期，无论是否被消费了。
4. kafka存储的消息是k-v键值对，k是offset偏移量，v就是消息的内容
5. broker：已发布的消息保存在一组服务器中，称之为Kafka集群。集群中的每一个服务器都是一个代理(Broker)。消费者可以订阅一个或多个主题（topic），并从Broker拉数据，从而消费这些已发布的消息。
6. topic：kafka将消息分门别类，每一类的消息称之为topic
7. patition：topic是逻辑上的概念，patition是物理概念，1个topic可以由1个或多个patition组成。1个partittion中的消息是队列形式的，每个消息被分了一个序列号offset，保证顺序
...
- 配置`kafka server`的`listeners`为本机ip
    ...
    listeners=PLAINTEXT://192.168.199.153:9092
    advertised.listeners=PLAINTEXT://192.168.199.153:9092
    ```
...
- 开始消费，会自动创建名称为test的topic：

    ```bash
    ./kafka-console-consumer.sh --bootstrap-server 192.168.199.153:9092 --topic test --from-beginning
    ```
...
```bash
./kafka-consumer-groups --bootstrap-server 10.112.208.70:9092,10.112.208.209:9092,10.112.208.210:9092 --describe -group test-group
# output
GROUP           TOPIC           PARTITION  CURRENT-OFFSET  LOG-END-OFFSET  LAG             CONSUMER-ID                                  HOST            CLIENT-ID
test-group      fee             0          -               6260            -               rdkafka-33525215-2d62-48bc-8dcc-afd0d536ef21 /10.122.163.122 rdkafka
```

这样我们就对kafka就有了一直相对直观的了解，最后我们把producer和consumer都取消。
...
- filebeat是一个日志文件托运工具，在你的服务器上安装客户端后，filebeat会监控日志目录或者指定的日志文件，追踪读取这些文件（追踪文件的变化，不停的读），并且转发这些信息到elasticsearch或者logstarsh或者kafka。
