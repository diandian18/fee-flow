...
<body>
    ...
    <script>
        // 模拟uuid
        if (!localstorage.get('custom_UUID')) {
            const customUUID = Math.random().toString().slice(-6);
            localstorage.set('custom_UUID', customUUID);
        }
        window.dt && dt.set({
            pid: 'test_project', // [必填]项目id, 由灯塔项目组统一分配
            uuid: localstorage.get('custom_UUID') || '', // [可选]设备唯一id, 用于计算uv数&设备分布. 一般在cookie中可以取到, 没有uuid可用设备mac/idfa/imei替代. 或者在storage的key中存入随机数字, 模拟设备唯一id.
...
3. 观察浏览器控制台是否报错，并上报了gif请求
    ...
    ```bash
    General
        Request URL: https://ferms.bestsign.info/dig.gif?a=1
        Request Method: GET
        Status Code: 200 OK
    ```
